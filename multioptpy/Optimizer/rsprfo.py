...
class EnhancedRSPRFO:
    def __init__(self, **config):
        ...
        with specialized mode following and eigenvector following techniques for DFT calculations
    ...
    def calculate_eigenvector_overlap_matrix(self, eigvecs_prev, eigvecs_curr):
        """
        Calculate overlap matrix between previous and current eigenvectors
        
        Parameters:
        eigvecs_prev: numpy.ndarray - Previous step eigenvectors as column vectors
        eigvecs_curr: numpy.ndarray - Current step eigenvectors as column vectors
        
        Returns:
        numpy.ndarray - Overlap matrix with elements S_{ij} = |<v_i^prev|v_j^curr>|
        """
        if eigvecs_prev is None or eigvecs_curr is None:
            return None
            
        # Matrix element S_{ij} = |<v_i^prev|v_j^curr>|
        overlap = np.abs(np.dot(eigvecs_prev.T, eigvecs_curr))
        return overlap
    ...
    def run(self, geom_num_list, B_g, pre_B_g=[], pre_geom=[], B_e=0.0, pre_B_e=0.0, pre_move_vector=[], initial_geom_num_list=[], g=[], pre_g=[]):
        """
        Execute one step of enhanced RSPRFO optimization
        ...
        # Execute Mode Following if enabled and this is a saddle point search
        ...
        # Calculate predicted energy change for convergence assessment
        predicted_energy_change = self.rfo_model(gradient, H, move_vector)
        self.predicted_energy_changes.append(predicted_energy_change)
        self.log(f"Predicted energy change: {predicted_energy_change:.6f}")
        
        # Store current geometry and gradient for next Hessian update
        self.prev_geometry = copy.deepcopy(geom_num_list)
        self.prev_gradient = copy.deepcopy(B_g)
    ...
    def update_hessian(self, current_geom, current_grad, previous_geom, previous_grad):
        ...
        # DFT-specific: Skip update if dot product is negative or zero
        if dot_product <= 0:
        ...
        if "flowchart" in self.hessian_update_method.lower():
            ...
            self.log(f"Unknown Hessian update method: {self.hessian_update_method}. Using auto selection.")
            # DFT specific: default to Bofill for saddle points, BFGS for minimization
            if self.saddle_order > 0:
                delta_hess = self.hessian_updater.Bofill_hessian_update(
                    self.hessian, displacement, delta_grad
                )
