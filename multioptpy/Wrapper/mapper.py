...
class ReactionNetworkMapper:
    def __init__(
        ...
        ts_checker: StructureChecker | None = None,
        ...
        rng_seed: int = 42,
        eq_energy_tolerance_kcalmol: float = 1.0,
        ts_energy_tolerance_kcalmol: float = 1.0,
    ) -> None:
        """
        Parameters
        ----------
        ...
        structure_checker : StructureChecker | None
            RMSD-based duplicate detector for **EQ (minimum) structures**.
            Defaults to ``StructureChecker(rmsd_threshold=0.30)``.
        ts_checker : StructureChecker | None
            Separate RMSD-based duplicate detector for **TS (saddle-point)
            structures**. Defaults to ``StructureChecker(rmsd_threshold=0.30)``.
        ...
        eq_energy_tolerance_kcalmol : float
            Energy window [kcal/mol] used when deciding whether two EQ
            structures are the same node.  Default: 1.0 kcal/mol.
        ts_energy_tolerance_kcalmol : float
            Energy window [kcal/mol] used as a pre-filter before comparing TS
            geometries for duplication.  Only applied when **both** TS energies
            are known; the filter is skipped entirely when either energy is
            ``None``.  Default: 1.0 kcal/mol.
        """
        self.base_config = base_config
        self.queue     = queue if queue is not None else BoltzmannQueue()
        # EQ and TS checkers are intentionally separate: TS saddle-point
        # geometries require a tighter RMSD threshold to avoid conflating
        # genuinely distinct transition states.
        self.checker    = structure_checker if structure_checker is not None else StructureChecker(rmsd_threshold=0.30)
        self.ts_checker = ts_checker        if ts_checker        is not None else StructureChecker(rmsd_threshold=0.30)
        ...
        # Separate energy tolerances for EQ nodes and TS edges.
        self.eq_energy_tolerance = eq_energy_tolerance_kcalmol / HARTREE_TO_KCALMOL
        self.ts_energy_tolerance = ts_energy_tolerance_kcalmol / HARTREE_TO_KCALMOL
    ...
    def _process_profile(self, profile_dir: str, run_dir: str) -> None:
        ...
        # Keep ts_energy as None when unavailable.
        # Previously this was coerced to 0.0, which caused the energy
        # pre-filter to pass trivially for every pair of TSes with unknown
        # energy, leading to false-positive duplicate detection.
        ...
        # ── TS duplicate check ─────────────────────────────────────────────
        # Performed BEFORE allocating an edge_id so that the counter is not
        # advanced for discarded duplicates.
        #
        # Design notes:
        #   • self.ts_checker uses a tighter RMSD threshold (default 0.15 Å)
        #     than the EQ checker (0.30 Å).  TS saddle-point geometries are
        #     inherently more similar to one another than EQ geometries, so a
        #     looser threshold leads to false-positive duplicate suppression.
        #   • The energy pre-filter is only applied when BOTH energies are
        #     known.  When either is None the filter is skipped entirely; the
        #     geometry check alone determines uniqueness.  This is safer than
        #     the previous behaviour of coercing None → 0.0, which made the
        #     filter trivially true for all TSes with unknown energy.
        try:
            ts_sym, ts_coords = parse_xyz(result["ts_xyz_file"])
            for existing_edge in self.graph.all_edges():
                if not (existing_edge.ts_xyz_file
                        and os.path.isfile(existing_edge.ts_xyz_file)):
                    continue

                # Energy pre-filter: skip when either energy is unknown.
                if ts_energy is not None and existing_edge.ts_energy is not None:
                    if abs(ts_energy - existing_edge.ts_energy) >= self.ts_energy_tolerance:
                        continue  # energies differ enough — cannot be the same TS

                ex_sym, ex_coords = parse_xyz(existing_edge.ts_xyz_file)
                if self.ts_checker.are_similar(ts_sym, ts_coords, ex_sym, ex_coords):
                    logger.debug(
                        "Duplicate TS detected (matches TS%06d, RMSD < %.3f Å). "
                        "Skipping edge registration for EQ%d--EQ%d.",
                        existing_edge.edge_id,
                        self.ts_checker.rmsd_threshold,
                        node_id_1,
                        node_id_2,
                    )
            ...
            logger.debug(
                "TS duplicate check failed for profile %s; "
                "registering as new edge to be safe.",
        ...
        edge = TSEdge(
            ...
            ts_energy=ts_energy,    # stored as None when unavailable
    ...
    def _find_or_register_node(
        ...
        for existing in self.graph.all_nodes():
            ...
            if energy is not None and existing.energy is not None:
                if abs(energy - existing.energy) > self.eq_energy_tolerance:
                    continue
