...
def update_trust_radii(B_e, pre_B_e, pre_B_g, pre_move_vector, model_hess, geom_num_list, trust_radii, trust_radii_update):
    if trust_radii_update == "trust":
        Sc = 2.0
        Ce = (np.dot(pre_B_g.reshape(1, len(geom_num_list)*3), pre_move_vector.reshape(len(geom_num_list)*3, 1)) + 0.5 * np.dot(np.dot(pre_move_vector.reshape(1, len(geom_num_list)*3), model_hess), pre_move_vector.reshape(len(geom_num_list)*3, 1)))
        r = (pre_B_e - B_e) / (Ce)
