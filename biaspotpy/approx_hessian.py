...
from parameter import UnitValueLib, number_element, covalent_radii_lib, UFF_effective_charge_lib, UFF_VDW_distance_lib, UFF_VDW_well_depth_lib, atomic_mass, D2_VDW_radii_lib, D2_C6_coeff_lib, D2_S6_parameter
...
class SwartD2ApproxHessian:
            ...
            for j in range(len(coord)):
                ...
                for k in range(len(coord)):
                    ...
                    for l in range(len(coord)):
                        ...
                        t_xyz = np.array([t_xyz_1, t_xyz_2, t_xyz_3, t_xyz_4])
                        theta, c = self.outofplane2(t_xyz)
    ...
    def outofplane2(self, t_xyz):
        ...
        if ((c14[2][0] ** 2 + c14[2][1] ** 2 + c14[2][2] ** 2) < self.eps):
            teta = 0.0
            bt = 0.0
            return teta, bt
    ...
    def torsion2(self, t_xyz):
        r_ij_1, b_r_ij = self.stretch2(t_xyz[0:2])
        r_ij_2, b_r_jk = self.stretch2(t_xyz[1:3])
        r_ij_3, b_r_kl = self.stretch2(t_xyz[2:4])
       
        fi_2, b_fi_2 = self.bend2(t_xyz[0:3])
        fi_3, b_fi_3 = self.bend2(t_xyz[1:4])
    ...
    def bend2(self, t_xyz):
        r_ij_1, b_r_ij = self.stretch2(t_xyz[0:2])
        r_jk_1, b_r_jk = self.stretch2(t_xyz[1:3])
        c_o = 0.0
        crap = 0.0
        for i in range(3):
