...
class BITSSModelFunction:
    ...
    def calc_energy(self, energy_1, energy_2, geom_num_list_1, geom_num_list_2, gradient_1, gradient_2, iter):
        ...
        if iter % 100 == 0:
            ...
            self.kappa_e = self.alpha / (2.0 * self.E_B)
            ...
            a = np.sqrt(np.linalg.norm(proj_grad_1) + np.linalg.norm(proj_grad_2)) / (2 ** 1.5 * self.beta * self.d)
            b = self.E_B / (self.beta * self.d ** 2)
            self.kappa_d = max(a, b)
        
        self.d = (1.0 - self.f) * self.d
