...
from fileio import save_bias_pot_info
...
class BiasPotentialCalculation:
    ...
    def main(self, e, g, geom_num_list, element_list,  force_data, pre_B_g="", iter="", initial_geom_num_list=""):
        ...
        #-----------------
        for j in range(len(bias_pot_obj_list)):
            tmp_B_e = bias_pot_obj_list[j].calc_energy(geom_num_list)
            tmp_tensor_BPA_grad = torch.func.jacfwd(bias_pot_obj_list[j].calc_energy)(geom_num_list)
            tmp_tensor_BPA_grad = tensor2ndarray(tmp_tensor_BPA_grad)
            tmp_tensor_BPA_hessian = torch.func.hessian(bias_pot_obj_list[j].calc_energy)(geom_num_list)
            ...
            save_bias_pot_info(self.BPA_FOLDER_DIRECTORY, tmp_B_e.item(), tmp_tensor_BPA_grad, bias_pot_obj_id_list[j])
